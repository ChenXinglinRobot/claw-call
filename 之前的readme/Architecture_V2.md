这份多场景动态路由与沙盒隔离架构的设计，彻底将前端的“皮”与后端的“魂”解耦。无论是用于高强度的英语词汇训练，还是用于温情的长辈自传访谈与怀旧疗法记录，系统都能基于这套架构做到游刃有余、互不干扰。

以下是为你整理的完整架构说明与分步修改指南 Markdown 文档，你可以直接将其保存为项目技术文档（如 `Architecture_V2.md`）。

***

# 实时语音对讲系统：多场景动态路由与沙盒隔离架构说明
...
系统抛弃了项目根目录下的全局 `prompt_config.json`，转而在 `memory/` 目录下建立严格的“基于用户的物理沙盒隔离区”。
...
│   └── /interview_project/
│       └── prompt.json                <-- 默认的长辈自传访谈/怀旧疗法设定
│
└── /users/                            <-- 动态读写区：飞书用户独立沙盒
    ├── /ou_a1b2c3d4e5f6/              <-- 示例：你的真实飞书 user_id
    │   ├── status.json                <-- 核心状态灯 (例: {"active_project": "vocab"})
    │   ├── status.tmp                 <-- OpenClaw 原子写入时的临时文件
    │   │
    │   ├── /vocab_project/            <-- 英语学习完全独立空间
    │   │   ├── prompt.json            <-- 词汇专属设定 (OpenClaw 维护)
    │   │   └── memory_log.md          <-- 单词对话历史记录
    │   │
    │   └── /interview_project/        <-- 访谈记录完全独立空间
    │       ├── prompt.json            <-- 访谈专属设定 (OpenClaw 维护)
    │       └── memory_log.md          <-- 长辈口述历史记录
    │
...
3.  在 `memory/users/test_user_123/` 建立测试沙盒，并写入 `status.json` 内容为 `{"active_project": "vocab"}`。
...
2.  逻辑链路同步修改：读取 `status.json` -> 确定活跃项目 -> 打开 `/memory/users/{user_id}/{project_name}/memory_log.md` -> 以 `mode='a'` (追加) 方式异步写入对话。

**验证标准**：
* 在 `vocab` 状态下完成一轮对话并挂断，检查单词的历史记录文件，确认追加成功。
* 手动切换至 `interview` 状态，聊几句日常，挂断。检查访谈的历史记录文件，确认内容精准写入，两个文件无任何交叉污染。

### 阶段四：交接控制权给 OpenClaw

**执行目标**：彻底剥离人工干预，确立 OpenClaw 的统治地位。
1.  在 OpenClaw 的 System Prompt 或 Tool 函数描述中，增加严格的文件操作约束规则。
2.  **强制规则定义**：明确告诉 OpenClaw，修改状态或 Prompt 必须采用“原子操作”。即：先写入 `.tmp` 后缀的临时文件，再执行系统级的 `mv`（或等效的重命名命令）覆盖原文件。
